...
def punctuation_auto_generate_subtitle_preprocessing(response, word_table):
    ...
    new_word_table['cut_para'] = np.where(new_word_table['word'] =='，' , 'cut', 
                                                    np.where(new_word_table['word'] =='？','cut' ,'no'))

    new_word_table = new_word_table.reset_index().drop(columns = 'index')

    # # 將有「cut」的index抓出來，方便我們切段落
    auto_select_range = new_word_table[new_word_table['cut_para'] == 'cut'].index
...
def punctuation_auto_generate_subtitle_second_adj(new_word_table, sec):
    ...
    for i in range(0, len(cut_point)):
        ...
        if i == 0:
            ...
            end = new_word_table_tmp.iloc[len(new_word_table_tmp)-1]['end_time']
            start = new_word_table_tmp.iloc[0]['start_time']

            # 如果句讀之間的區間大於sec秒，就要cut
            if end- start >= sec:
                new_word_table_tmp['cut_para'] = np.where( ( new_word_table_tmp['start_time_floor'] != new_word_table_tmp['end_time_floor'] ) & (new_word_table_tmp['end_time_floor'] % sec == 0) , 'cut',  new_word_table_tmp['cut_para'])
        
        elif i != len(cut_point) - 1:
            new_word_table_tmp = new_word_table[cut_point[i-1]:cut_point[i]]
            end = new_word_table_tmp.iloc[len(new_word_table_tmp)-1]['end_time']
            start = new_word_table_tmp.iloc[0]['start_time']
        ...
        # 最後一個
            ...
            new_word_table_tmp = new_word_table[cut_point[i-1] : len(new_word_table) ]
            end = new_word_table_tmp.iloc[len(new_word_table_tmp)-1]['end_time']
            start = new_word_table_tmp.iloc[0]['start_time']
...
def auto_generate_subtitle(word_table, auto_select_range, video):
    ...
    for i in range(0, len(auto_select_range)):
        ...
        if i==0:
            ...
            temp = word_table[  (word_table.index>=0)  & (word_table.index<autotemp) ]
            ...
            # 影片特製秒數轉換
            sentence_start_time, sentence_end_time = srt_sec_format(sentence_start_time, sentence_end_time)
            
            auto_select_list.append([word_cut,sentence_start_time,sentence_end_time])             
        ...
        # 第二個後的斷句，僅抓後面的斷字區間 [17 ~25], [25 ~ 32]...
            ...
            if i != len(auto_select_range)-1:
                ...
                temp = word_table[  (word_table.index>=autotemp[0])  & (word_table.index<autotemp[1]) ]
            ...
            # 如果是最後一個數字，就執行else
                ...
                sentence_end_time= temp['end_time'].loc[autotemp+len(temp)-1]
...
def auto_generate_subtitle_preprocessing(response, sec_split):
    ...
    for i in response:#.results:
        ...
        for word in i.alternatives[0].words:
            ...
            word_ = word.word
            # 抓取開始時間
            s_time = round(word.start_time.seconds + word.start_time.microseconds * 1e-6, 3)
            # 抓取結束時間
            e_time = round( word.end_time.seconds + word.end_time.microseconds * 1e-6, 3)
            # 將結果一並添加至word_list中
            word_list.append([word_,s_time,e_time])

    word_table = pd.DataFrame(word_list, columns = ['word', 'start_time', 'end_time'])
    ...
    word_table['cut_para'] = np.where( ( word_table['start_time_floor'] != word_table['end_time_floor'] ) & (word_table['end_time_floor']% sec_split == 0) , 'cut',  'no')
...
def srt_sec_format(sentence_start_time, sentence_end_time):
    ...
    if np.floor(sentence_start_time) != sentence_start_time :
        # 情況一:該秒數含有小數點調整方法
        sentence_start_time= '0'+str(datetime.timedelta(seconds=sentence_start_time))[:-3]
    else :
        # 情況二:該秒數不含小數點調整方法
        sentence_start_time= '0'+str(datetime.timedelta(seconds=sentence_start_time))+'.000'
    
    # 結束時間處理
    if np.floor(sentence_end_time) != sentence_end_time :
        # 情況一:該秒數含有小數點調整方法
        sentence_end_time= '0'+str(datetime.timedelta(seconds=sentence_end_time))[:-3]
    else :
        # 情況二:該秒數不含小數點調整方法
        sentence_end_time = '0'+str(datetime.timedelta(seconds=sentence_end_time))+'.000'
...
def video2text_basic(video_path):
    ...
    clip.audio.write_audiofile(video_path.split('.')[0]+'.wav',
                            fps=48000, # 頻率
                            ffmpeg_params=["-ac", "1"]
                            ) # 編碼方式
...
def long_speech_recognition_extra_function(
    ...
    #-------------- 長語音轉字檔 -----------
    # 上傳檔案
    upload_blob(bucket_name=bucket_name, source_file_name=speech_path, destination_blob_name=destination_blob_name,json_os=json_os)
    ...
    speech_contexts = {"phrases": speech_contexts,
 "boost": 20}
    ...
    if enable_word_confidence:
        ...
        word_frame = pd.DataFrame( {'斷字': text_list, 
                '機器認字信心水準':conf_list,
                '始末時間':time_list})

        word_frame['優先查看排序'] = word_frame['機器認字信心水準'].rank(method= 'min')

        word_frame.to_csv(destination_blob_name + '_逐字信心水準表.csv', encoding='utf-8-sig')
    ...
    if enable_speaker_diarization:
        ...
        for i in rec:
            ...
            # print(i.word)
            # 開始時間
            s_time.append(i.start_time.seconds + i.start_time.microseconds*1e-6)
            # 結束時間
            e_time.append(i.end_time.seconds + i.end_time.microseconds*1e-6)


        # 轉為DataFrame
        speaker_frame = pd.DataFrame(list(zip(speaker, text_list, s_time, e_time)),
        ...
        speaker_frame.to_csv(destination_blob_name + '_不同談話者區分表.csv', encoding='utf-8-sig')
        ...
        for i in range(0, len(speaker_frame_printdf['說話者編號'])):
            ...
            if i == (len(speaker_frame_printdf['說話者編號'])-1):
                print('結束')
                result.append(speaker_frame_printdf['說話者編號'].iloc[i] + '說：「' +  ''.join(wording) +'」')
                wording = []


            # 本次的i == 下一個i，也就是還是同一個人的話，就還是歸納成一個人
            elif speaker_frame_printdf['說話者編號'].iloc[i] == speaker_frame_printdf['說話者編號'].iloc[i+1]:
        ...
        wording.to_csv(destination_blob_name + '_不同談話者講話內容.csv', encoding='utf-8-sig')
    ...
    if audio_channel_count == 1:
        ...
        confid_frame = pd.DataFrame(list(zip(recog_list, recog_confidence, recog_start, recog_end)),
                                    columns =['文章段句', '機器認字信心水準', '開始時間', '結束時間'])

        confid_frame['改善順序'] = confid_frame['機器認字信心水準'].rank(ascending = True, method = 'min').astype(int)

        if enable_xlsx:
            confid_frame.to_csv(destination_blob_name + '_文章認字信心矩陣.csv', encoding='utf-8-sig')
        ...
        for i in range(0,confid_frame.shape[0]):
            timer_translist.append(str('【'+str(confid_frame.iloc[i,2])+' to '+str(confid_frame.iloc[i,3])+'】'))

        document = Document()
        document.styles['Normal'].font.name = u'宋體' # 設定字體
        document.styles['Normal']._element.rPr.rFonts.set(qn('w:eastAsia'), u'宋體') # 設定字體
        document.add_heading('文章逐字稿', 0) # 設定標題
        document.add_paragraph('機器認字信心水準' + str(round(confid_frame['機器認字信心水準'].mean(),2) ) + '\n\n' ) # 添加段落 － 信心水準


        for i in range(0,confid_frame.shape[0],1):
            document.add_paragraph(confid_frame['文章段句'].iloc[i] + timer_translist[i])

        if enable_wordcloud:
            document.add_picture(destination_blob_name+'_文字雲.png',width=Cm(15), height=Cm(13))
